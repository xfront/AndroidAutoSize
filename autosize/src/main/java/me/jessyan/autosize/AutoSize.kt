...
object AutoSize {
    ...
    fun checkInit(): Boolean {
        return AutoSizeConfig.initDensity != -1f
    }
    ...
    fun checkAndInit(application: Application) {
        if (!checkInit()) {
            AutoSizeConfig.setLog(true).apply {
                init(application)
    ...
    fun autoConvertDensityOfGlobal(activity: Activity) {
        if (AutoSizeConfig.isBaseOnWidth) {
            autoConvertDensityBaseOnWidth(activity, AutoSizeConfig.designWidthInDp.toFloat())
        } else {
            autoConvertDensityBaseOnHeight(activity, AutoSizeConfig.designHeightInDp.toFloat())
        }
    }
    ...
    fun cancelAdapt(activity: Activity) {
        ...
        setDensity(
            activity, AutoSizeConfig.initDensity,
